...
const checkParticipantTopicCreated = () => {
    ...
    if (postCreated) {
        ...
        if (refresh.length) {
            ...
            Api.request(endpoint, requestData)
                .get()
                .done(response => {
...
function setFinished(event) {
    ...
    Api.request(endpoint, { finished: true })
        .patch()
        .done(
            response => {
                $(this)
            ...
            response => {
                ...
                if (response.status === 403) {
                    if (window.console) {
                        console.log(
                            "Attempting to change other project to finished"
                        );
                    }
                } else {
                    alert(
                        "Er ging iets fout, neem contact op met admins@modelbrouwers.nl. Fout: " +
