...
import AlbumSelectionErrorModal from './AlbumSelectionErrorModal';
...
const initUpload = () => {
    ...
    uploader = new qq.FineUploader({
      ...
      callbacks: {
        onComplete: function (event, succeeded, failed) {
          if (failed.length === 0) {
            window.location = decodeURI(uploadSettings.albumDetail).replace('{0}', album, 1);
          }
        },
        onSubmit: function () {
          const ok = setAlbum();
  ...
  const setAlbum = () => {
    ...
    if (checked.length !== 1) {
      intlProviderPropsPromise
        .then(intlProviderProps => {
          modalRoot.render(
            <IntlProvider {...intlProviderProps}>
              <AlbumSelectionErrorModal onRequestClose={onCloseModal} />
            </IntlProvider>,
          );
        })
        .catch(console.error);
