...
const KitPreview = ({htmlName, inputType, kit, selected = false, onToggle = () => {}}) => {
  ...
  return (
    <div className="col-xs-12 col-sm-4 col-md-3 col-xl-2 preview">
      <input
        ...
        onChange={onChange}
      />
      <label htmlFor={`__modelkit_${kit.id}`} title={kit.name} className="thumbnail">
        <img src={thumbImg} className="img-responsive" onError={onThumbError} alt="boxart" />
        <span className="h5">
          <strong>
            {kit.name}
