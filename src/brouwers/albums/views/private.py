...
class MyAlbumsView(LoginRequiredMixin, TemplateView):
    ...
    def get_shared_albums(self):
        ...
        return Album.objects.select_related("cover").filter(
            albumgroup__users=user, trash=False
        )

    def get_my_shared_albums(self):
        user = self.request.user
        return (
            user.album_set.select_related("cover")
            .exclude(albumgroup__isnull=True)
            .exclude(trash=True)
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["tabcontent"] = OrderedDict(
            sorted(_tab_content.items(), key=lambda t: tab_order.index(t[0]))
        )
        return context
