...
class IndexView(FormMixin, ListView):
    queryset = (
        KitReview.objects.select_related("reviewer", "model_kit")
        .annotate_mean_rating()
...
class ReviewListView(SingleObjectMixin, ListView):
    queryset = (
        KitReview.objects.prefetch_related(
            Prefetch(
                "ratings",
                queryset=KitReviewPropertyRating.objects.select_related("prop"),
            )
        )
...
class KitReviewDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["other_reviews"] = (
            self.object.model_kit.kitreview_set.select_related("reviewer", "album")
            .annotate_mean_rating()
