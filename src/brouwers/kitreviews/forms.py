...
class FindModelKitForm(forms.Form):
    brand = forms.ModelChoiceField(
        queryset=Brand.objects.all(), label=_("Brand"), required=False
    )
    ...
    scale = forms.ModelChoiceField(
        queryset=Scale.objects.all(), label=_("Scale"), required=False
    )
...
class KitReviewForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["model_kit"].queryset = self.fields[
            "model_kit"
...
class KitReviewPropertyRatingForm(AlwaysChangedModelForm):
    class Meta:
        ...
        widgets = {"rating": RangeInput(attrs={"max": MAX_RATING})}
