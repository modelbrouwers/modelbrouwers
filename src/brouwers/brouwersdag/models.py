...
class ShowCasedModel(models.Model):
    ...
    remarks = models.TextField(
        _("remarkable elements"),
        blank=True,
        help_text=_(
            'Add the features that make this model special here, e.g. "scratch built cockpit"'
        ),
    )
    ...
    length = models.PositiveSmallIntegerField(
        _("length"), null=True, blank=True, help_text=_("In cm.")
    )
    width = models.PositiveSmallIntegerField(
        _("width"), null=True, blank=True, help_text=_("In cm.")
    )
    height = models.PositiveSmallIntegerField(
        _("height"), null=True, blank=True, help_text=_("In cm.")
    )

    # competition?
    competition = models.ForeignKey(
        "brouwersdag.Competition",
        null=True,
        blank=True,
...
class Competition(models.Model):
    ...
    price = models.DecimalField(
        _("price per model"), max_digits=5, decimal_places=2, default="0.0"
    )

    max_num_models = models.PositiveSmallIntegerField(
        _("models per participant"),
        default=0,
        help_text=_("Maximum number of models per participant, enter 0 for unlimited."),
    )
    max_participants = models.PositiveSmallIntegerField(
        _("maximum number of participants"),
        default=0,
        help_text=_("Maximum number of participants, enter 0 for unlimited."),
    )

    is_current = models.BooleanField(
        _("current open competition?"),
        default=False,
        help_text=_(
            "Marking this competition as active will deactivate all other competitions."
        ),
    )
...
class Exhibitor(models.Model):
    ...
    brouwersdag = models.ForeignKey(
        Brouwersdag, blank=True, null=True, on_delete=models.SET_NULL
    )
    space = models.CharField(
        _("space needed"),
        blank=True,
        max_length=100,
        help_text=_("Amount of space needed. 100 characters or less."),
    )
