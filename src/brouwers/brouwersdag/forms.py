...
class ShowCasedModelSignUpForm(forms.ModelForm):
    error_messages = {
        "limit_person": _(
            "There is a limit of {limit} model(s) per person for this competition."
        ),
        "limit_participants": _(
            "The maximum number of contestors is reached. You can no longer enter the competition"
        ),
    }
        ...
        if owner and limit_per_person:
            ...
            if num >= limit_per_person:
                raise forms.ValidationError(
                    self.error_messages["limit_person"].format(limit=limit_per_person)
                )
