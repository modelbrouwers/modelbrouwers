    ...
    def form_valid(self, form):
        ...
        if self.object.rejected:
            messages.info(
                request,
                _(
                    "The builder of this project doesn't participate in the awards."
...
class NominationListView(ListView):
    ...
    def get_queryset(self):
        ...
        return Nomination.objects.filter(
            category__id=category.id, nomination_date__year=date.today().year
...
def scores(request):
    ...
    for category in categories:
        projects = (
            Project.objects.filter(category__exact=category, nomination_date__year=year)
            .exclude(rejected=True)
            .order_by("-votes")
        )
        votes_total = 0
        if projects:
            ...
            data.append(
                {"category": category, "projects": projects[:5], "total": votes_total}
            )
    return render(
        request,
        "awards/vote_scores.html",
        {"data": data, "year": year, "voters": voters},
    )
...
class VoteView(LoginRequiredMixin, TemplateView):
    ...
    def get_forms(self, data=None):
        ...
        categories = (
            projects.select_related("category")
            .distinct("category")
            .order_by("category")
    ...
    def forms_valid(self, forms):
        ...
        for category_name, formdata in forms.items():
            ...
            if form.is_valid():
                ...
                messages.success(
                    self.request,
                    _("Your vote for `%(category)s` was saved.")
        ...
        if has_errors:
            messages.error(
                self.request,
                _(
                    "One or multiple category votes could not be saved. "
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not _voting_enabled():
            ...
            vote_end_date = date(
                this_year + 1, settings.VOTE_END_MONTH, settings.VOTE_END_DAY
            )
...
class WinnersView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # list of years
        context["years"] = (
            Nomination.objects.exclude(rejected=True)
            .filter(votes__gt=0)
    ...
    def get_winners(self, year):
        ...
        nominations = (
            Nomination.objects.filter(nomination_date__year=year, rejected=False)
